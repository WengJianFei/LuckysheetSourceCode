...
const server = {
    ...
    openWebSocket: function(){
        ...
        method.createHookFunction('cooperativeMessage', data)
				...
				console.log(JSON.parse(data.data), '后台响应数据')
                  ...
                  setTimeout(() => {
					...
					if(flag) {
						Store.cooperativeEdit.changeCollaborationSize.forEach(val => {
							if(val.id == id) {
								val.v = item.v[0] || (item.v.range ? item.v.range[0] : item.range[0])
								val.i = index
							}
						})
					} else {
	        ...
	        _this.websocket.onclose = function(e){
				...
				if(e.code === 1000){
					...
					message.info(locale().websocket.contact);
				}
	        }
	    }
	    else{
					message.info(locale().websocket.support);
	    }
    },
