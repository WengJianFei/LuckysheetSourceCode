...
export function initialMatrixOperation(){
    ...
    $("#luckysheet-copy-json-head").click(function (event) {
        ...
        if(Store.luckysheet_select_save.length > 1){
            if(isEditMode()){
                message.info(locale_drag.noMulti);
            }
            else{
        ...
        if(Store.config["merge"] != null){
            ...
            if(has_PartMC){
                if(isEditMode()){
                    message.info(locale_drag.noPartMerge);
                }
                else{
    ...
    $("#luckysheet-copy-arraymore-confirm").click(function (event) {
        ...
        if(row.toString() == "NaN" || col.toString() == "NaN"){
            if(isEditMode()){
                message.info(locale_drag.inputCorrect);
            }
            else{
        ...
        if(row < 1 || col < 1){
            if(isEditMode()){
                message.info(locale_drag.notLessOne);
            }
            else{
    ...
    $("#luckysheet-copy-diagonaloffset").click(function (event) {
        ...
        if(offset < 0){
            if(isEditMode()){
                message.info(locale_drag.offsetColumnLessZero);
            }
            else{
